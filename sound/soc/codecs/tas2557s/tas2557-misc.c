...
static ssize_t tas2557_file_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    printk("tas2557_file_write: p_kBuf[0]=%x\n", p_kBuf[0]);
	...
	case TIAUDIO_CMD_FW_RELOAD_24:
	{
		printk("TIAUDIO_CMD_FW_RELOAD_24: count=%ld\n", count);
		if (count == 1) {
			ret = request_firmware_nowait(THIS_MODULE, 1, TAS2557_FW_NAME_24,
				pTAS2557->dev, GFP_KERNEL, pTAS2557, tas2557_fw_ready);

			//if (g_logEnable)
			printk("TIAUDIO_CMD_FW_RELOAD_24: ret = %d\n",ret);
		}
	}
	break;
