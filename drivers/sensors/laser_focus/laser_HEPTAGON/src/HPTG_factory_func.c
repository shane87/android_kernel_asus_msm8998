...
*
*/
uint16_t Laura_device_read_range2(struct msm_laser_focus_ctrl_t *dev_t, int cal_type)
{
	uint16_t RawRange, Range = 0, i2c_read_data = 0, error_status = 0;
	uint16_t RawConfidence = 0, confidence_level =0;
	uint16_t IT_verify = 0;
	...
	while(1){
		...
		if(dev_t->elisa_fw19) {
			status = CCI_I2C_WrByte(dev_t, 0x18, 0x06);
			status = CCI_I2C_WrByte(dev_t, 0x19, 0x20);
			status = CCI_I2C_RdWord(dev_t, I2C_DATA_PORT, &IT_verify);
		} else {
		...
		LOG_Handler(LOG_CDBG, "%s: reg 0x08: 0x%04x\n", __func__, RawRange);
		...
		LOG_Handler(LOG_CDBG,"%s: conf(%d) IT_verify(%s:%d)\n", __func__, confidence_level, dev_t->elisa_fw19? "0x2006" : "0x2030", IT_verify);
		LOG_Handler(LOG_CDBG, "%s: Range(%d) ErrCode(%d) RawRange(%x)\n", __func__, Range, error_status >> 13, RawRange);	

		if(cal_type == MSM_LASER_FOCUS_APPLY_OFFSET_CALIBRATION) {
...
*
*/
int Laura_device_calibration(struct msm_laser_focus_ctrl_t *dev_t, int16_t *cal_input_data, int cal_type)
{
	int status = 0, pass_count = 0, fail_count = 0, i = 0, j = 0;
	uint16_t distance = 0;
	int16_t cal_data[CAL_MSG_LEN_ELISA];
	uint16_t i2c_read_data;
	uint8_t mailbox_K_cmd, cal_msg_len;
	struct timeval start,now;

	mailbox_K_cmd = GET_ELISA_CALIBRATION;
	cal_msg_len = CAL_MSG_LEN_ELISA;
	
	O_get_current_time(&start);

	//LOG_Handler(LOG_FUN, "%s: Enter\n", __func__);

	LOG_Handler(LOG_CDBG, "%s: Calibration type: %d\n", __func__, cal_type);
	...
	do{
		/* Read distance */
		distance = Laura_device_read_range2(dev_t, cal_type);
		...
		pass_count += 1;
		LOG_Handler(LOG_CDBG, "%s: Pass count: %d!!\n", __func__, pass_count);
		
		/* CMD_MBX to read data */
		status = Olivia_Mailbox_Command(dev_t, mailbox_K_cmd, cal_data, cal_msg_len);
		if(status < 0){
...
int Laser_get_calibration_input(struct seq_file *buf, void *v, 
	int16_t *cal_data_10, int16_t *cal_data_40,  int16_t *cal_data_f0, int16_t * ConfPoint){
	

	int i=0;
	int cal_msg_fo, cal_msg_len;

	cal_msg_fo = CAL_MSG_F0_ELISA;
	cal_msg_len = CAL_MSG_LEN_ELISA;
