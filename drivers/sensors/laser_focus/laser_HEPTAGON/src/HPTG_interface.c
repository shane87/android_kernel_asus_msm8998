...
extern const struct sErrLimitConfig gsDefaultErrLimitCfg;
...
*/
int16_t cal_data_10[CAL_MSG_LEN_ELISA*2];
int16_t cal_data_40[CAL_MSG_LEN_ELISA*2];
int16_t cal_data_inf[CAL_MSG_LEN_ELISA*2];	/* Calibration infinity input data */
int16_t cal_data_f0[CAL_MSG_F0_ELISA];
...
int	Do_TOF_and_Calibration(struct msm_laser_focus_ctrl_t *dev_t, enum laura_configuration_ctrl  cal_type, int16_t* cal_data){
	...
	/* Do calibration */
	status = Laura_device_calibration(dev_t, cal_data, cal_type);
	if(status < 0)
		return status;
...
int Laser_calibration_interface(struct msm_laser_focus_ctrl_t *dev_t, bool load_cal, bool *calibration_flag, int ctrl){
	...
	if(calib[0]&&calib[1]){
		calib[0] = 0;
		calib[1] = 0;
		Olivia_DumpKdata(dev_t, cal_data_f0, CAL_MSG_F0_ELISA);
		Tof_configuration_interface(dev_t, MSM_LASER_FOCUS_APPLY_NORMAL_CALIBRATION);
	}
...
//		status = CCI_I2C_WrWord(dev_t, ICSR, PAD_INT_MODE);
		status = CCI_I2C_WrWord(dev_t, ICSR, 0);
	...
	//if(status ==0 && dev_t->conditional_measurement)
	//    status = hptg_set_err_limit_config(dev_t, gsDefaultErrLimitCfg);
...
int Laser_wake_up_interface(struct msm_laser_focus_ctrl_t *dev_t, bool load_cal, bool *cal_flag){
	...
	if(status ==0) {
		if(!dev_t->cal_start && dev_t->operation_mode == OP_MODE_CUSTOM) {
			status = ambient_setting(dev_t);

			if(status == 0)
				status = hptg_set_intG_amplitude_ratio(dev_t);

			if(status == 0 && dev_t->conditional_measurement && load_cal)        
				status = hptg_set_cont_meas_ready_event_param(dev_t, dev_t->u16MinimumThreshold_mm, dev_t->u16MaximumThreshold_mm, dev_t->eMode);

			if(status == 0 && dev_t->enable_proximity)
				status = hptg_config_proximity(dev_t);
		}
	}
