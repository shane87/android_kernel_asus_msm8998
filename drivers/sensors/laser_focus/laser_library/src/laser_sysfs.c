...
int Sysfs_read_int(char *filename, size_t size)
{
	...
	if (IS_ERR_OR_NULL(fp)) {
		...
		return -ENXIO;	/*No such file or directory*/
	}
	...
	if (fp->f_op != NULL) {
		pos_lsts = 0;
		readlen = vfs_read(fp, buf, size, &pos_lsts);
		if(readlen < 0) {
			LOG_Handler(LOG_ERR, "%s: File read (%s) fail\n", __func__, filename);
			set_fs(old_fs);
			filp_close(fp, NULL);
			return -5000;
		}
		...
		LOG_Handler(LOG_ERR, "%s: File (%s) fp->f_op == NULL\n", __func__, filename);			
		return -ENXIO;
	}
...
int Sysfs_read_word_seq(char *filename, int *value, int size)
{
	...
	if (fp->f_op != NULL) {
		...
		pos_lsts = 0;
		for(i = 0; i < size; i++){
			ret = vfs_read(fp, buf[i], 5, &pos_lsts);
			if(ret < 0) {
				LOG_Handler(LOG_ERR, "%s: File read (%s) fail\n", __func__, filename);
				set_fs(old_fs);
				filp_close(fp, NULL);				
...
int Sysfs_read_Dword_seq(char *filename, int *value, int size)
{
	...
	if (fp->f_op != NULL) {
		pos_lsts = 0;
		for(i = 0; i < size; i++){
			ret = vfs_read(fp, buf[i], 9, &pos_lsts);
			if(ret < 0) {
				LOG_Handler(LOG_ERR, "%s: File read (%s) fail\n", __func__, filename);
				set_fs(old_fs);
				filp_close(fp, NULL);
...
bool Sysfs_write_int(char *filename, int value/*, umode_t mode*/)
{
	...
	if (fp->f_op != NULL) {
		pos_lsts = 0;
		ret = vfs_write(fp, buf, strlen(buf), &fp->f_pos);
		if(ret < 0) {
			LOG_Handler(LOG_ERR, "%s: File write (%s) fail\n", __func__, filename);
			set_fs(old_fs);
			filp_close(fp, NULL);			
			return -EIO;
		}
...
bool Sysfs_write_word_seq(char *filename, uint16_t *value, uint32_t size)
{
	...
	if (fp->f_op != NULL) {
		pos_lsts = 0;
		for(i = 0; i < size; i++){
			ret = vfs_write(fp, buf[i], 5, &fp->f_pos);
			if(ret < 0) {
				LOG_Handler(LOG_ERR, "%s: File write (%s) fail\n", __func__, filename);
				set_fs(old_fs);
				filp_close(fp, NULL);				
...
bool Sysfs_write_Dword_seq(char *filename, uint32_t *value, uint32_t size)
{
	...
	if (fp->f_op != NULL) {
		pos_lsts = 0;
		for(i = 0; i < size; i++){
			ret = vfs_write(fp, buf[i], 9, &fp->f_pos);
			if(ret < 0) {
				LOG_Handler(LOG_ERR, "%s: File write (%s) fail\n", __func__, filename);
				set_fs(old_fs);
				filp_close(fp, NULL);				
