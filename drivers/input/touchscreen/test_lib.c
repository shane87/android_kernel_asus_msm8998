...
struct Test_ConfigParam_FT5X46{
	...
	boolean PanelDifferTest;
	short PanelDifferTest_Min;
	short PanelDifferTest_Max;
	...
	short PannelDifferTest_Low_Min_node[TX_NUM_MAX][RX_NUM_MAX];	
	short PannelDifferTest_Low_Max_node[TX_NUM_MAX][RX_NUM_MAX];
...
/*test section*/
#define Item_VIRTUAL_BUTTON_TEST "VIRTUAL_BUTTON_TEST"
#define Item_RF_VK_TEST "RF_VK_TEST"

#define Item_Set_Key_Num "Set_Key_Num"
#define Item_LimitTime_Key "LimitTime_Key"
#define Item_Use_Key_Value "Use_Key_Value"

#define Item_MAX_NG_Key "MAX_NG_Key"
#define Item_Key1_Left "Key1_Left"
#define Item_Key1_Top "Key1_Top"
#define Item_Key1_Right "Key1_Right"
#define Item_Key1_Bottom "Key1_Bottom"
#define Item_Key2_Left "Key2_Left"
#define Item_Key2_Top "Key2_Top"
#define Item_Key2_Right "Key2_Right"
#define Item_Key2_Bottom "Item_Key2_Bottom"
#define Item_Preserved_key_threshold_Dynamic "Item_Preserved_key_threshold_Dynamic"
#define Item_SET_TOUCH_THRESHOLD_INCELL "SET_TOUCH_THRESHOLD_INCELL"
#define Item_Key_Div_Number_Incell "Key_Div_Number_Incell"
#define Item_Preserved_key_threshold_Incell "Preserved_key_threshold_Incell"

#define Item_SET_TOUCH_THRESHOLD_INCELL2 "SET_TOUCH_THRESHOLD_INCELL2"
#define Item_Key_Div_Number_Incell2 "Key_Div_Number_Incell2"
#define Item_Preserved_key_threshold_Incell2 "Preserved_key_threshold_Incell2"
...
#define PANELDIFFERTest "PanelDifferTest"
#define PANELDIFFERTest_Max "PanelDifferTest_Max"
#define PANELDIFFERTest_Min "PanelDifferTest_Min"

#define Special_PANELDIFFER_Min "Panel_Differ_Min_Tx"
#define Special_PANELDIFFER_Max "Panel_Differ_Max_Tx"
...
int m_DifferData[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
int m_absDifferData[TX_NUM_MAX][RX_NUM_MAX] = {{0}};
boolean m_bV3TP;
...
//static boolean TestItem_PanelDifferTest(void);
...
int TestItem_visual_key(void);
int TestItem_rf_visual_key(void);
...
static void GetTestParam(void)
{
	...
	g_TestParam_FT5X46.VIRTUAL_BUTTON_TEST = GetParamValue(Section_TestItem,Item_VIRTUAL_BUTTON_TEST,0);;
	g_TestParam_FT5X46.RF_VK_TEST = GetParamValue(Section_TestItem,Item_RF_VK_TEST,0);
    g_TestParam_FT5X46.Set_Key_Num = GetParamValue(Section_BaseSet,Item_Set_Key_Num,2);

    g_TestParam_FT5X46.LimitTime_Key = GetParamValue(Section_BaseSet,Item_LimitTime_Key,10);
 g_TestParam_FT5X46.Use_Key_Value= GetParamValue(Section_BaseSet,Item_Use_Key_Value,0);

g_TestParam_FT5X46.Key_Left[0]= GetParamValue(Section_BaseSet,Item_Key1_Left,599);
g_TestParam_FT5X46.Key_Top[0]= GetParamValue(Section_BaseSet,Item_Key1_Top,1999);
g_TestParam_FT5X46.Key_Right[0]= GetParamValue(Section_BaseSet,Item_Key1_Right,601);
g_TestParam_FT5X46.Key_Bottom[0]= GetParamValue(Section_BaseSet,Item_Key1_Bottom,2001);
g_TestParam_FT5X46.Key_Left[1]= GetParamValue(Section_BaseSet,Item_Key2_Left,199);
g_TestParam_FT5X46.Key_Top[1]= GetParamValue(Section_BaseSet,Item_Key2_Top,1999);
g_TestParam_FT5X46.Key_Right[1]= GetParamValue(Section_BaseSet,Item_Key2_Right,201);
g_TestParam_FT5X46.Key_Bottom[1]= GetParamValue(Section_BaseSet,Item_Key2_Bottom,2001);


g_TestParam_FT5X46.bDynamicHole=GetParamValue(Section_BaseSet,Item_Preserved_key_threshold_Dynamic,1);
g_TestParam_FT5X46.bSetTouchThreshold[0]=GetParamValue(Section_BaseSet,Item_SET_TOUCH_THRESHOLD_INCELL,0);
g_TestParam_FT5X46.iDivNumer[0]=GetParamValue(Section_BaseSet,Item_Key_Div_Number_Incell,1);
g_TestParam_FT5X46.iPerservedKey[0]=GetParamValue(Section_BaseSet,Item_Preserved_key_threshold_Incell,4);

g_TestParam_FT5X46.bSetTouchThreshold[1]=GetParamValue(Section_BaseSet,Item_SET_TOUCH_THRESHOLD_INCELL2,1);
g_TestParam_FT5X46.iDivNumer[1]=GetParamValue(Section_BaseSet,Item_Key_Div_Number_Incell2,1);
g_TestParam_FT5X46.iPerservedKey[1]=GetParamValue(Section_BaseSet,Item_Preserved_key_threshold_Incell2,10);
if ((g_TestParam_FT5X46.VIRTUAL_BUTTON_TEST)||(g_TestParam_FT5X46.RF_VK_TEST))
	return;
	//zax 20141116-----------------------------
#if (FT5X46_TEST == 1)
	...
	//zax 20150112-------------------------------------------
	g_TestParam_FT5X46.PanelDifferTest = GetParamValue(Section_TestItem, PANELDIFFERTest, 0);
	g_TestParam_FT5X46.PanelDifferTest_Min = GetParamValue(Section_BaseSet, PANELDIFFERTest_Min, 500);
	g_TestParam_FT5X46.PanelDifferTest_Max = GetParamValue(Section_BaseSet, PANELDIFFERTest_Max, 3000);
	...
	for (i = 0; i < TX_NUM_MAX; i++) 
	{
		for (j = 0; j < RX_NUM_MAX; j++) 
		{
			...
			g_TestParam_FT5X46.PannelDifferTest_Low_Min_node[i][j]=g_TestParam_FT5X46.PanelDifferTest_Min;
			g_TestParam_FT5X46.PannelDifferTest_Low_Max_node[i][j]=g_TestParam_FT5X46.PanelDifferTest_Max;
	...
	for (i = 0; i < TX_NUM_MAX; i++) 
	{
		for (j = 0; j < RX_NUM_MAX; j++) 
		{
			...
			g_TestParam_FT5X46.InvalidNodes[i][j] = GetParamValue(Section_INVALID_NODE, str_tmp, 1);
		}
	}
	...
	///////////////////////////////// Panel_Differ_Min_Tx1

	for (i = 0; i < TX_NUM_MAX; i++) 
	{		
		memset(str_value, 0x00, sizeof(str_value));
		memset(str_tmp, 0x00, sizeof(str_tmp));
		sprintf(str_tmp, "%s%d",Special_PANELDIFFER_Min,(i+1));
		valuelen = GetParamString(Section_SpecialSet, str_tmp, str_value);
		...
		if (valuelen > 0) 
		{
			...
			for(j=0; j<valuelen; j++) 
			{
				if(',' == str_value[j]) 
				{
					g_TestParam_FT5X46.PannelDifferTest_Low_Min_node[i][k] = (short)(atoi(str_tmp));
					index = 0;
					memset(str_tmp, 0x00, sizeof(str_tmp));
					k++;
				} 
...
///////////////////////////////// Panel_Differ_Max_Tx
	...
	{		
		memset(str_value, 0x00, sizeof(str_value));
		memset(str_tmp, 0x00, sizeof(str_tmp));
		sprintf(str_tmp, "%s%d",Special_PANELDIFFER_Max,(i+1));
		valuelen = GetParamString(Section_SpecialSet, str_tmp, str_value);
		...
		if (valuelen > 0) 
		{
			...
			for(j=0; j<valuelen; j++) 
			{
				if(',' == str_value[j]) 
				{
					g_TestParam_FT5X46.PannelDifferTest_Low_Max_node[i][k] = (short)(atoi(str_tmp));
					index = 0;
					memset(str_tmp, 0x00, sizeof(str_tmp));
					k++;
				} 
...
boolean FT5X46_TestItem(void)
{
	...
	if(g_TestParam_FT5X46.VIRTUAL_BUTTON_TEST)
	{
		...
		gpio_direction_output(ftxxxx_ts->pdata->rst_gpio, 0);
    	       msleep(20);
              gpio_direction_output(ftxxxx_ts->pdata->rst_gpio, 1);
              msleep(200);
         printk("[focal] %s vk_test_result = %d  \n", __func__, bTestReuslt);
		return bTestReuslt;

	}
	if(g_TestParam_FT5X46.RF_VK_TEST)
	{
		mutex_unlock(&ftxxxx_ts->g_device_mutex);
		wake_unlock(&ftxxxx_ts->wake_lock);
		bTestReuslt = bTestReuslt &TestItem_rf_visual_key();
		gpio_direction_output(ftxxxx_ts->pdata->rst_gpio, 0);
    	msleep(20);
       gpio_direction_output(ftxxxx_ts->pdata->rst_gpio, 1);
      msleep(200);
	  printk("[focal] %s vk_rf_test_result = %d  \n", __func__, bTestReuslt);
      return bTestReuslt;
	}
...
static boolean TestItem_UniformityTest(void)
{
	...
	bool bResult;
	boolean bIncludeKey;
	ReCode = ReadReg( 0x00, &strSwitch );
	printk("[2]0x00 ReCode = %d \n", strSwitch);
...
static boolean TestItem_PanelDifferTest(void)
{
	...
	printk("Reg_normalize type = %d \n", OriginRawDataType);
	if( ReCode < ERROR_CODE_OK)	
	{
			FTS_DBG("read reg_normalize_type error !\n");
			btmpresult = false;
	}
	if(OriginRawDataType  !=0){
		ReCode =WriteReg(REG_NORMALIZE_TYPE,0);
		ReadReg(REG_NORMALIZE_TYPE,&OriginRawDataType );
		printk("Reg_normalize type = %d \n", OriginRawDataType);
		if( ReCode < ERROR_CODE_OK)	
		{
			FTS_DBG("write reg_normalize_type error !\n");
			btmpresult = false;
		}
	...
	for(i = 0; i <iTxNum; i++)
	{
		for( j= 0; j < iRxNum; j++)
		{
			m_DifferData[i][j] = rawdata[i][j]/10;
		}
	}
	...
	*/
	...
	/*iMin =  g_TestParam_FT5X46.PanelDifferTest_Min;		//	g_stCfg_FT5X22_BasicThreshold.PanelDifferTest_Min[iRow][iCol];
	iMax = g_TestParam_FT5X46.PanelDifferTest_Max;
	printk("[touch]iMax = %d iMin = %d \n ",iMax,iMin);*/
	for(iRow = 0; iRow<iTxNum; iRow++)	//	iRow = 1 ???
	{
		for(iCol = 0; iCol < iRxNum; iCol++)
		{
			if(g_TestParam_FT5X46.InvalidNodes[iRow][iCol] == 0)continue;//Invalid Node
			iMin =  g_TestParam_FT5X46.PannelDifferTest_Low_Min_node[iRow][iCol];	//	g_stCfg_FT5X22_BasicThreshold.PanelDifferTest_Min[iRow][iCol];
			iMax = g_TestParam_FT5X46.PannelDifferTest_Low_Max_node[iRow][iCol];
			iValue = m_DifferData[iRow][iCol];
			if(iValue < iMin || iValue > iMax)
			{
				btmpresult = false;
				FTS_DBG("Out Of Range.  Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d) \n", \
	...
	//	strTemp += "\r\n";
		...
		//	strTemp += str;
			...
			//if( NODE_AST_TYPE == g_TestParam_FT5X46.InvalidNodes[i][j] || NODE_INVALID_TYPE == g_TestParam_FT5X46.InvalidNodes[i][j])
			if( NODE_INVALID_TYPE == g_TestParam_FT5X46.InvalidNodes[i][j])
			{
				InvalidNum++;
				continue;
			}
...
//	SaveTestData(strTemp, m_iTxNum, m_iRxNum);
	//Save_Test_Data(m_absDifferData, 0, iTxNum, iRxNum, 1);		
	////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<收集测试数据，存入CSV文件

	AvgValue = AvgValue/( iTxNum*iRxNum - InvalidNum); 
	FTS_DBG("PanelDiffer:Max: %d, Min: %d, Avg: %d ", maxValue, minValue,AvgValue);
	ReCode =WriteReg(REG_NORMALIZE_TYPE,OriginRawDataType);
	if( ReCode < ERROR_CODE_OK)	
		{
			FTS_DBG("Failed to restore normalize type!");
			btmpresult = false;
		}
	//恢复v3屏体的mapping值
	if (m_bV3TP)
	{
		...
		if( ReCode != ERROR_CODE_OK)	
		{
			FTS_DBG("Failed to restore mapping type!");
			btmpresult = false;
		}
	}
	/////////////
	...
	{
		bTestResult = true;
		FTS_DBG("		//Panel Differ Test is OK!");
	}
	else
	{
		bTestResult = false;
		FTS_DBG("		//Panel Differ Test is NG!");
	}
	return bTestResult;
}
#endif
...
{    
	...
	for(i = 0; i < g_TestParam_FT5X46.Set_Key_Num; i++)    
	{        
		if((x >= g_TestParam_FT5X46.Key_Left[i])            
			&& (x <= g_TestParam_FT5X46.Key_Right[i])            
			&& (y >= g_TestParam_FT5X46.Key_Top[i])            
			&& (y <= g_TestParam_FT5X46.Key_Bottom[i]))        
		{            
			vk_result[i] = 1;            
		pr_err("[touch]vk result[%d]=%d \n", i, vk_result[i]);        
		}    
	}            
	pr_err("[touch]%s end \n", __func__);   
	return 0;
...
//    int j = 0;
    ...
    int time_cycle = 0;
   // struct i2c_client *client = ftxxxx_ts->client;
//    struct i2c_client *client = (struct i2c_client *)ftxxxx_ts->client;
 	FTS_DBG("%s start", __func__);


    if (0 == g_TestParam_FT5X46.Set_Key_Num)
        return ERROR_CODE_INVALID_PARAM;
    //enable_irq(client->irq);
    memset(vk_result, 0, sizeof(int)*8);
    if(1 == g_TestParam_FT5X46.bDynamicHole)
    {   
		ReadReg( 0x80, &reg_val);
		m_lastTouchThreshold = reg_val;
		if(1 == g_TestParam_FT5X46.bSetTouchThreshold[1])
		{
			reg_val = reg_val * (g_TestParam_FT5X46.iPerservedKey[1]*10+100)/100;
			WriteReg(0x80, reg_val);
		}		
    }
    else
    {
		...
		if(1 == g_TestParam_FT5X46.bSetTouchThreshold[1])
		{
			reg_val = g_TestParam_FT5X46.iPerservedKey[1];	
			WriteReg(0x80, reg_val);
		}
    }
 //    FTS_TEST_DBG("touchthreshold = %d", reg_val);

    time_cycle = g_TestParam_FT5X46.LimitTime_Key * 1000/500;
    ...
    {
       /* for(j = 0; j < g_TestParam_FT5X46.Set_Key_Num; j++)
        {
            if(vk_result[j] == 1)
            {
                pr_err("[touch]vk%d wait %d \n", j, vk_result[j]);
                break;
            }
	     else
	     {
	         pr_err("[touch]vk%d touch %d \n ", j, vk_result[j]); 		
	     }
        }
        if (j < g_TestParam_FT5X46.Set_Key_Num)
        {
            msleep(500);
            continue;
        }
	...
	visual_key_active = 0;
    WriteReg(0x80, m_lastTouchThreshold);
    pr_err("[ouch]%s i=%d end \n", __func__, i);
   memset(vk_result, 0, sizeof(int)*8);
   ...
   ftxxxx_nosync_irq_disable(ftxxxx_ts->client);
    ...
    {
    //    *bTestResult = false;
	 //FTS_TEST_INFO("FT5822_TestItem_visual_key is NG");
	 //TestResultLen += sprintf(TestResult+TestResultLen,"FT5822_TestItem_visual_key is NG. \n\n");
        return 1;//2001ERROR_CODE_INVALID_PARAM;
    }
    else
    {
      //  *bTestResult = true;
	// FTS_TEST_ERROR("FT5822_TestItem_visual_key is OK");
	 //TestResultLen += sprintf(TestResult+TestResultLen,"FT5822_TestItem_visual_key is OK. \n\n");
        return  0;//ERROR_CODE_OK;
    }
}
int TestItem_rf_visual_key(void)
{
